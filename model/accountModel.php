...
function get_accounts(PDO $db, Array $user):?Array {
  $query = $db->prepare(
    "SELECT a.id, a.amount, a.creation_date, a.account_type, o.operation_amount, o.operation_date
     FROM Account AS a
     LEFT JOIN (
    	  SELECT o1.* FROM Operation as o1
        LEFT JOIN Operation AS o2
        ON o1.operation_account_id = o2.operation_account_id
        AND o1.id < o2.id
        WHERE o2.id IS NULL
    ) AS o
    ON a.id = o.operation_account_id
    WHERE account_customer_id = :account_customer_id"
  );
  $query->execute([
    "account_customer_id" => $user["id"]
  ]);
...
function get_single_account($db, $id) {
  $query = $db->prepare(
    "SELECT a.*, o.id AS operation_id, o.operation_type, o.operation_amount, o.operation_date FROM Account AS a
...
function get_only_account($db, $id, $user) {
  $query = $db->prepare(
    "SELECT id, amount FROM Account
     WHERE id = :id
     AND account_customer_id = :account_customer_id"
   );
  $query->execute([
    "id" => $id,
    "account_customer_id" => $user["id"]
...
function new_account($db, $account, $user) {
  $query = $db->prepare(
    "INSERT INTO Account(amount, creation_date, account_type, account_customer_id)
    VALUES(:amount, NOW(), :account_type, :account_customer_id)"
  );
